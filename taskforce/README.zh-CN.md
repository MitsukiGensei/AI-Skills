...
- **验证前移，不堆到最后**：触及 UI / 场景 / 资产 / 运行时注册的包，必须在自己的闸门上先过本包冒烟，跑在**本工作区自己的** Editor 上。波次级的集成验证只保留跨包链路，超过 8 条或预计半小时就分片，修完只复测失败项而不从第一条重跑。报缺陷必须带**真实调用路径**的复现和根因证据；同一缺陷第二次修复复测仍失败就停止派修，交给实证 probe，不做第三次盲修。
- **一个工作区就是一个工程**：技能会先探自己跑在哪种形态——从池里写时复制克隆出的 worktree（自己的 P4 client、自己的 `.taskforce/`、自己的 Editor）、传统 P4 单 workspace、还是纯 git——再据此决定隔离原语、早锁与收口方式。worktree 形态下一切读写都留在本工作区内：不借别的 worktree 的 recon、任务板和 Editor；和兄弟工作区唯一剩下的耦合是同一条 depot stream，派工前跑一次 `p4 opened -a` 看清楚即可，不发起跨会话的文件所有权谈判。
- **PM 是单例**：会话可能在原进程还活着时被同一份 transcript 重新拉起，于是两个 PM 对着同一批 subagent 重复派工、交替覆写同一批文件。所以 PM 开工就写一把带自己进程号的锁，并在每次 resume 标记后重新核对：原进程还在跑，本会话就自报是副本并停下，而不是去协商。
- **有预算，因为账户是共享的**：Claude subagent 和主会话吃同一份额度，并行的 taskforce 会话更是叠在同一个池上。所以上限写死——每波在跑的 opus/fable ≤ 3，整机跨任务 ≤ 4；每个派出去的 agent 都带轮次与时间预算，到点就落盘并回报"做完了什么、还剩什么"，而不是硬撑到做完。长跑 agent 每完成一个可独立单元就追加落盘，被打断后只重派没落盘的部分。
...
1.3.0 到 1.5.0 版（2026-09-01/02）的改动不来自论文，来自我们自己的复盘：一次任务把 Editor 验证全堆在最后，4 轮 Play 占掉 45% 的墙钟；以及某天三个并行任务同时撞上账户 session limit、零收口。代价记在 `references/evidence.md`，改出来的结果就是上面那几条运行时验证、工作区形态、PM 锁与预算规则。
...
| `references/convergence.md` | 收敛回路细则（预扫描清单、review 视角清单、交叉收敛的 prompt 骨架与归并工作表、codex 派单 prompt 骨架、运行时验证 prompt 骨架与缺陷记录格式、完成报告模板），只在跑到对应环节时读 |
| `references/codex.md` | codex 编队附录：恒定配置、manifest 形态、编队纪律与实测签名，起编队或排障时才读 |
| `references/evidence.md` | 规则的来处：论文七条失效模式对照本 skill 的防线、带实测签名的修订史，以及 1.3–1.5 版背后的事故数据 |
