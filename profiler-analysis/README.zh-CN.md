...
本目录除技能本体外，还随附了**Unity 工程侧的采样/导出配套**（AI Profiler 面板、导出器、通用运行时采集器、无人值守菜单、Lua 后端抽象与 Miku 适配、纯 Lua 打点适配器），见 [unity/README.md](unity/README.zh-CN.md)。技能负责"导出之后"的分析，`unity/` 负责"导出之前"的采样——两者配套才是完整链路。
...
`unity/` 子目录是 Unity 工程侧配套，不需要进技能目录；按 [unity/README.md](unity/README.zh-CN.md) 合入工程即可（复制技能时可一并带上、也可剔除，不影响技能运行）。
...
| `unity/` | Unity 工程侧配套（采样 + 导出），详见 [unity/README.md](unity/README.zh-CN.md) |
...
`AI Profiler` 面板（`Window/Analysis/AI Profiler`，源码见 [unity/](unity/README.zh-CN.md)）顶部可切**采样模式**：
...
| 依赖 | 说明 |
|---|---|
| Unity「AI Profiler」面板 | `Window/Analysis/AI Profiler`，多源整合导出；支持 Editor 本地 / 真机连接两种采样模式。源码与合入方式见 [unity/README.md](unity/README.zh-CN.md) |
| Lua 后端（可选） | 有 Lua 的工程接 `ILuaProfilerBackend`；参考实现为 MikuLuaProfiler 适配（上游 <https://github.com/leinlin/Miku-LuaProfiler>，本仓库不附带其源码，见 [unity/Miku-LuaProfiler/README.md](unity/Miku-LuaProfiler/README.zh-CN.md)）。无 Lua 的工程 Lua 维度为 NO DATA |
...
- **Unity 侧**：宏、打点、Lua 后端、冲突插桩关闭接入点见 [unity/README.md](unity/README.zh-CN.md)。
...
- **Unity 侧脚本变** → 同步 [unity/README.md](unity/README.zh-CN.md) 的文件清单与接线点；面板私有方法改名时 `AIProfilerAutomation.cs` / `AIProfilerSkills.cs` 的反射调用会显式报错，一并修。
- **README 改动** → `README.md`（英文，默认）与 `README.zh-CN.md`（中文）两版同步更新；本目录树下所有 README 同此约定。
