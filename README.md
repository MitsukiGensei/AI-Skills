...
| Skill | Description |
|---|---|
| [taskforce](taskforce/README.md) | Requirement-delivery engine (project-manager mode): the main agent acts only as PM — sizes the task, splits it into work packages, dispatches multi-tier model subagents in parallel (+ Codex as external help), converges quality through objective gates and independent review, and collects the changes into a P4 pending changelist for the user to sign off; state lives in an external task board so it survives frequent context compaction |
